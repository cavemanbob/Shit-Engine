...
/*	InitPawnAttacks();
 ...
 char* benchmarkfens[52] = {
	...
	for(int i =0; i < 52; i++){
		ApplyFen(&x, benchmarkfens[i]);
		u64 movecount = 0;
		output m = movegen(x);
		for(int j = 0; j < m.from.size(); j++){
			movecount += Perft(FromTo(x, m.from[i], m.to[i], m.PieceType[i]), 4);
		}
		std::cout << i << "-  " << movecount << std::endl;
	}
	//ApplyFen(&x,str);
	//ReadableBoard(x);
	//output m = movegen(x);
	...
	//r2qkb1r/5pp1/p1p2n1p/2pp2N1/4p1b1/2N1P3/PPPP1P1P/R1BQK2R w KQkq - 0 13
	//position x;
	//ApplyFen(&x, START_FEN);
	//x = FromTo(x, 8, 24, PAWN_W);
	//std::cout << (int)x.to_square;
