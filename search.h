...
float alphabeta(position b, int depth, float alpha, float beta){
	Node_Total++;
	if(depth == 0){
		//if(is_square_attacked(b, b.to_square, (b.key >> 8 & 1ULL) ? BLACK : WHITE) == 1 && (b.key >> 31 & 1ULL) == 0ULL){// moved a dangerous square
/*		if(b.key >> 31 & 1ULL && b.overlap < 2){ // captured
			//ReadableBoard(b);
			//std::cout << (int)b.to_square << std::endl;
			//std::cout << b.to_square << " " << is_square_attacked(b, b.to_square) << std::endl;
			//_sleep(450);
			//b.key |= 1ULL << 31;
			b.overlap++;
			return alphabeta(b, 1, alpha, beta);
		}*/
		return Evaluate(b);
	}
